...
It is often appropriate(合适的、恰当的) to reuse a single object instead of creating a new functionally equivalent(相同的、相等的) object each time it is needed. Reuse can be both faster and more stylish. An object can always be reused if it is immutable (Item 17).

复用单个对象通常是合适的，不必每次需要时都创建一个新的功能等效对象。复用可以更快、更流行。如果一个对象是总是不可变，那么它一直都可以被复用（[Item-17](/Chapter-4/Chapter-4-Item-17-Minimize-mutability.md)）。
